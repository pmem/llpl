...
public class Heap1Tests {
	...
	public void testCreateHeapFused() {
		...
		heap = Heap.createHeap(TestVars.POOL_SET_FILE, 0);
	...
	public void testCreateFixedHeap() {
		heap = (TestVars.ISDAX) ? TestVars.createHeap()
					            : Heap.createHeap(TestVars.HEAP_USER_PATH + TestVars.HEAP_NAME, TestVars.HEAP_SIZE);
        Assert.assertTrue(Heap.exists(TestVars.HEAP_USER_PATH + TestVars.HEAP_NAME));
		Assert.assertTrue(heap.size() > 0);
		if (!TestVars.ISDAX) {
	...
	public void testCreateGrowableWithLimitHeap() {
		if (TestVars.ISDAX) throw new SkipException("Test not valid in DAX mode");
		Assert.assertFalse(Heap.exists(TestVars.HEAP_USER_PATH + TestVars.HEAP_NAME));
		Assert.assertTrue(TestVars.createFolder(TestVars.HEAP_USER_PATH + TestVars.HEAP_NAME));
		heap = Heap.createHeap(TestVars.HEAP_USER_PATH + TestVars.HEAP_NAME, TestVars.HEAP_SIZE);
		Assert.assertTrue(Heap.exists(TestVars.HEAP_USER_PATH + TestVars.HEAP_NAME));
		Assert.assertTrue(heap.size() > 0);
		File file = new File(TestVars.HEAP_USER_PATH + TestVars.HEAP_NAME);
	...
	public void testOpenGrowableWithLimitHeap() {
		...
		if (!file.exists() || !file.isDirectory())
			Assert.assertFalse(true);
		Assert.assertTrue(heapSize == heap.size());
	}
	...
	public void testMultipleOnGrowableNoLimitHeaps() {
		if (TestVars.ISDAX) throw new SkipException("Test not valid in DAX mode");
		Heap[] heaps = new Heap[TestVars.NUM_HEAPS];
		// set root
		for (int i = 0; i < TestVars.NUM_HEAPS; i++) {
		...
		MemoryBlock[] blocks = new MemoryBlock[TestVars.NUM_HEAPS];
		for (int i = 0; i < TestVars.NUM_HEAPS; i++) {
			int ii = i;
			if (i % 2 == 0)
	...
	public void testMultipleOnGrowableNoLimitHeapsExceedsMemory() {
		...
		try {
			heaps = new Heap[TestVars.NUM_HEAPS];
			...
			Assert.assertTrue(MAX_HEAPS_ALLOCATED < TestVars.NUM_HEAPS);
		} 
        finally {
		 	Assert.assertTrue(MAX_HEAPS_ALLOCATED < TestVars.NUM_HEAPS);
	...
	public void testMultipleOnGrowableWithLimitHeaps() {
		...
		for (int i = 0; i < TestVars.NUM_HEAPS; i++) {
			Assert.assertTrue(TestVars.createFolder(TestVars.HEAP_USER_PATH + TestVars.HEAP_NAME + i));
			heaps[i] = Heap.createHeap(TestVars.HEAP_USER_PATH + TestVars.HEAP_NAME + i, TestVars.HEAP_SIZE);
			heaps[i].setRoot((i + 1) * 1234);
		}
		// get root
		for (int i = 0; i < TestVars.NUM_HEAPS; i++) {
	...
	public void testMultipleOnGrowableWithLimitHeapsExceedsMemory() {
		...
		try {
			...
			for (int i = 0; i < TestVars.NUM_HEAPS; i++) {
				Assert.assertTrue(TestVars.createFolder(TestVars.HEAP_USER_PATH + TestVars.HEAP_NAME + i));
				heaps[i] = Heap.createHeap(TestVars.HEAP_USER_PATH + TestVars.HEAP_NAME + i, TestVars.HEAP_SIZE_3G);
				MAX_HEAPS_ALLOCATED++;
				heaps[i].setRoot((i + 1) * 1234);
				Assert.assertTrue(heaps[i].getRoot() == (i + 1) * 1234);
				MemoryBlock mb = null;
	...
	public void testOpenMemoryBlockNonTransactionalWrongHandleWithLargeSizeOpenGrowableNoLimitHeap() {
		heap = TestVars.createHeap();
		MemoryBlock mb = heap.allocateMemoryBlock(TestVars.MEMORY_BLOCK_SIZE, TestVars.NON_TRANSACTIONAL);
		Assert.assertTrue(mb.isValid());
    ...
    public void testHeapExecuteSupplier(){
        heap = TestVars.createHeap();
        MemoryBlock mb1 = heap.execute(() -> {
            MemoryBlock mbInternal = heap.allocateMemoryBlock(1024);
    ...
    @Test
    public void testAnyHeapExistsNullPath(){
        heap = TestVars.createHeap();
        try {
            Heap.exists(null);
    ...
    @Test
    public void testAnyHeapExistsInvalidDAXPath(){
        heap = TestVars.createHeap();
        Assert.assertFalse(Heap.exists("/dev/dax/path/to/invalid/heap"));
    }
