...
public class Employee {
    ...
    public Employee(Heap heap, String name, long id) {
        byte[] nameBytes = name.getBytes();
        block = heap.allocateMemoryBlock(NAME_OFFSET + nameBytes.length, false); 
    ...
    public static Employee fromHandle(Heap heap, long handle) {
        return new Employee(heap.memoryBlockFromHandle(handle));
    }
    
    private Employee(MemoryBlock block) {
        this.block =  block;
    }
